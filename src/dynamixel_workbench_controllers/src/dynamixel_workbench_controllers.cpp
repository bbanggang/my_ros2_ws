...
DynamixelController::DynamixelController()
   ...
   wheel_radius_(0.0f),
   is_moving_(false) //for traj. control
{   
  ...
  is_joint_state_topic_ = this->get_parameter("use_joint_states_topic").get_value<bool>();

  //is_cmd_vel_topic_ = priv_node_handle_.param<bool>("use_cmd_vel_topic", false);
  this->declare_parameter("use_cmd_vel_topic",true);
  is_cmd_vel_topic_=this->get_parameter("use_cmd_vel_topic").get_value<bool>();
  //use_moveit_ = priv_node_handle_.param<bool>("use_moveit", false);
  this->declare_parameter("use_moveit",false);
  use_moveit_=this->get_parameter("use_moveit").get_value<bool>();

  //read_period_ = priv_node_handle_.param<double>("dxl_read_period", 0.010f);
  this->declare_parameter("dxl_read_period",0.050f);
  read_period_=this->get_parameter("dxl_read_period").get_value<double>();

  //write_period_ = priv_node_handle_.param<double>("dxl_write_period", 0.010f);
  this->declare_parameter("dxl_write_period",0.050f);
  write_period_=this->get_parameter("dxl_write_period").get_value<double>();

  //pub_period_ = priv_node_handle_.param<double>("publish_period", 0.010f);
  this->declare_parameter("publish_period",0.050f);
  pub_period_=this->get_parameter("publish_period").get_value<double>();
  ...
  if (is_cmd_vel_topic_ == true)
  {
    ...
    this->declare_parameter("mobile_robot_config/seperation_between_wheels",0.287);
    wheel_separation_=this->get_parameter("mobile_robot_config/seperation_between_wheels").get_value<double>();
    //wheel_radius_ = priv_node_handle_.param<double>("mobile_robot_config/radius_of_wheel", 0.0);
    this->declare_parameter("mobile_robot_config/radius_of_wheel",0.033);
    wheel_radius_=this->get_parameter("mobile_robot_config/radius_of_wheel").get_value<double>();
  }

  dxl_wb_ = new DynamixelWorkbench;
  
  //jnt_tra_ = new trajectory_msgs::JointTrajectory;
  //jnt_tra_ = new trajectory_msgs::msg::JointTrajectory;
    
  //jnt_tra_msg_ = new trajectory_msgs::JointTrajectory;
  //jnt_tra_msg_ = new trajectory_msgs::msg::JointTrajectory;
  ...
  std::string yaml_file = "/home/rapi5/ros2_ws/src/dynamixel_workbench_controllers/config/basic.yaml";
  ...
  //read_timer_ = this->create_wall_timer(rclcpp::Duration(getReadPeriod()), std::bind(&DynamixelController::readCallback, this));
  read_timer_ = this->create_wall_timer(std::chrono::milliseconds((int64_t)(1000*getReadPeriod())), std::bind(&DynamixelController::readCallback, this));
  // for joint trajectory control
  //write_timer_ = this->create_wall_timer(rclcpp::Duration(getWritePeriod()), std::bind(&DynamixelController::writeCallback, this));
  //write_timer_ = this->create_wall_timer(std::chrono::milliseconds((int64_t)(1000*getWritePeriod())), std::bind(&DynamixelController::writeCallback, this));
  //publish_timer_ = this->create_wall_timer(rclcpp::Duration(getPublishPeriod()), std::bind(&DynamixelController::publishCallback, this));
  publish_timer_ = this->create_wall_timer(std::chrono::milliseconds((int64_t)(1000*getPublishPeriod())), std::bind(&DynamixelController::publishCallback, this));
...
bool DynamixelController::loadDynamixels(void)
{
  ...
  for (auto const& dxl:dynamixel_)
  {
    ...
    {      
      ...
      RCLCPP_INFO(this->get_logger(),"Name : %s, ID : %d, Model Number : %d", dxl.first.c_str(), dxl.second, model_number);
    }
  }
      ...
      if (dxl.first == info.first)
      {
        if (info.second.item_name != "ID" && info.second.item_name != "Baud_Rate")
        {
          ...
          if (result == false)
          {
            ...
            RCLCPP_ERROR(this->get_logger(),"Failed to write value[%d] on items[%s] to Dynamixel[Name : %s, ID : %d]", info.second.value, info.second.item_name.c_str(), dxl.first.c_str(), dxl.second);
            return false;
          }
        }
      }
...
void DynamixelController::initPublisher()
{
  ...
  dynamixel_state_list_pub_ = this->create_publisher<dynamixel_workbench_msgs::msg::DynamixelStateList>("dynamixel_state", qos_profile);
  
  //if (is_joint_state_topic_) joint_states_pub_ = priv_node_handle_.advertise<sensor_msgs::JointState>("joint_states", 100);
  if (is_joint_state_topic_) joint_states_pub_ = this->create_publisher<sensor_msgs::msg::JointState>("joint_states", qos_profile);
...
void DynamixelController::initSubscriber()
{
  ...
  auto qos_profile = rclcpp::QoS(rclcpp::KeepLast(10));
  //trajectory_sub_ = this->create_subscription<trajectory_msgs::msg::JointTrajectory>("joint_trajectory", qos_profile, std::bind(&DynamixelController::trajectoryMsgCallback,this,_1));
  ...
  if (is_cmd_vel_topic_) cmd_vel_sub_ = this->create_subscription<geometry_msgs::msg::Twist>("cmd_vel", qos_profile, std::bind(&DynamixelController::commandVelocityCallback,this,_1));
}

void DynamixelController::initServer()
{
  dynamixel_command_server_ = this->create_service<dynamixel_workbench_msgs::srv::DynamixelCommand>("dynamixel_command", std::bind(&DynamixelController::dynamixelCommandMsgCallback,this,std::placeholders::_1,std::placeholders::_2));
}
...
void DynamixelController::readCallback()
{
  ...
  dynamixel_workbench_msgs::msg::DynamixelState  dynamixel_state[dynamixel_.size()];
...
void DynamixelController::commandVelocityCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
  bool result = false;
  const char* log = NULL;
  ...
  RCLCPP_INFO(this->get_logger(),"linvel:%lf,angvel:%lf",robot_lin_vel,robot_ang_vel);
...
// for joint trajectory control
/*void DynamixelController::trajectoryMsgCallback(const trajectory_msgs::msg::JointTrajectory::SharedPtr msg)
{
  uint8_t id_cnt = 0;
  bool result = false;
  WayPoint wp;
...
bool DynamixelController::dynamixelCommandMsgCallback(const std::shared_ptr<dynamixel_workbench_msgs::srv::DynamixelCommand::Request> req,
                                                      const std::shared_ptr<dynamixel_workbench_msgs::srv::DynamixelCommand::Response> res)
{
  bool result = false;
  const char* log;
  ...
  if (result == false)
  {
    ...
    RCLCPP_ERROR(this->get_logger(),"Failed to write value[%d] on items[%s] to Dynamixel[ID : %d]", value, item_name.c_str(), id);
  }
